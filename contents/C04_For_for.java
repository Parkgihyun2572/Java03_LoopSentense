...
public class C04_For_for {
	public static void main(String[] args) {
		/* 이중 for문: 반복문 안에 반복문 */
		for (int i = 1; i <= 5; i++) {//내부for문 반복을 마치면 수행
			for (int j = 1; j <= 3; j++) {//반복 먼저 수행 
		...
		*/
		...
		System.out.println("외부for: 0<=i<4, 4번 반복범위");
		System.out.println("내부for: 0<=j<3, 3번 반복범위");
		...
		System.out.println("외부for: 0<=i<4, 4번 반복범위");
		System.out.println("내부for: 0<=j<i, i번 반복범위");
		count1 = 0; //외부for문 반복 횟수 체크
		count2 = 0; //내부for문 반복 횟수 체크
		for (int i = 0; i < 4; i++) {//4번 반복 조건
			count1++;
